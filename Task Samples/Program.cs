#region TASK И THREAD
/*
Task предоставляет более высокоуровневый и удобный способ управления 
асинхронными операциями по сравнению с непосредственной работой с потоками 
через Thread. Task автоматически использует пул потоков .NET, что позволяет 
эффективно управлять ресурсами и избегать создания большого количества 
ненужных потоков. При использовании Task ошибки внутри задачи агрегируются 
и возвращаются в виде исключений, что облегчает обработку ошибок.
Использование Task уменьшает вероятность появления многопоточных ошибок, 
таких как состояние гонки (race conditions), так как управление потоками 
происходит на уровне платформы.

Task представляет асинхронную операцию и обеспечивает легкую и удобную 
модель программирования для работы с параллельными задачами:

1. Асинхронность: 
Позволяет выполнять асинхронные операции в фоновом режиме, не блокируя 
основной поток выполнения программы. Это особенно важно в приложениях с 
пользовательским интерфейсом, чтобы не "замораживать" UI при выполнении 
долгих операций.
2. Параллелизм: 
Позволяет выполнять несколько задач параллельно, что может улучшить 
производительность в многопроцессорных системах.
3. Управление потоками: 
Позволяет запускать задачи в различных потоках или пулах потоков.
4. Комбинирование асинхронных операций: 
Позволяет комбинировать результаты нескольких асинхронных операций, 
используя методы ContinueWith, WhenAll и другие.
5. Улучшенный контроль над задачей: 
Позволяет ожидать завершения задачи, отменить её выполнение или 
установить ей приоритет.
6. Упрощение асинхронного кода: 
Появление ключевого слова `async/await` позволяет писать асинхронный
код более легко и читаемо, избегая сложного управления потоками.
*/
#endregion
...
Вызывать метод Dispose нужно только тогда, когда это имеет влияние на 
производительность, и задачи 100% более никем не используются
*/
#endregion
